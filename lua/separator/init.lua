...
function M.refresh()
    -- get all function_definition nodes from TreeSitter
    -- local result = require("vim.treesitter.query").set("lua", "", "(comment) @comment")
    ...
    for _, value in ipairs(result) do
        ...
        local line = tokens[1]
        local function_name = tokens[2]
        vim.print(line)
        vim.print(function_name)
        M.draw_separator(tonumber(line) + 1, M.config.class_style)
    end
    -- vim.print(result)
end
