...
function M.draw_separator(row, style)
    ...
    local line_length = (line == nil) and 0 or #line
    -- if line ~= nil then line_length = #line end
    local txt = string.rep(style.char, style.width - 1 - line_length)

    -- set a extmark
    local mark_id = vim.api.nvim_buf_set_extmark(M.BUFFER, M.mark_ns, row, 0, {
        virt_text = {
            { txt, style.name }
        }
    })
...
function M.refresh()
    ...
    backend.get_function_list(function(result)
        for _, item in ipairs(result) do
            local line = item[1]
            local function_name = item[2]
            M.draw_separator(tonumber(line) + 1, M.config.styles.class)
        end
    end)
end
